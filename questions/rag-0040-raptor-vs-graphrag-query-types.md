...
用 LLM 从文本抽取实体、关系，构成图；再对图做社区聚类并为每个社区生成摘要。检索分两路：局部问题以相关实体为入口，收集并排序其邻域的实体、关系、社区摘要与关联文本片段，利用实体邻域与关系上下文组织多跳证据；全局问题聚合社区摘要做 map-reduce。
...
擅长：
- **多跳关系推理**：「A 和 C 通过谁/什么产生联系」——借实体邻域与关系上下文，能串起 chunk 里分散、never co-occur 的事实。
...
因为二者的「全局」来路不同，成本与失真点也不同。GraphRAG 的全局总结依赖**实体抽取 + 社区检测**，对「关系稀疏、实体不清晰」的语料（如叙事散文、连续论证）抽图噪声大、构建贵，且抽取会丢掉非实体型语义。RAPTOR 只做**聚类 + 摘要**，不假设文本里有清晰实体关系，通常省去实体/关系抽取和社区检测，构建链路更简单，天然保留原文语义粒度；但它自身也有递归聚类与多轮摘要的开销，实际成本取决于语料规模、摘要层数与 GraphRAG 的配置，并非绝对更省。反过来，多跳关系问题 RAPTOR 就吃力——它的树中没有显式的关系边，多跳关系证据不如 GraphRAG 直接，只能靠摘要「碰巧」提到。所以判据是**语料结构**：强实体关系（知识库、事件网络）→ GraphRAG；弱关系、重层次归纳（报告、论文、书）→ RAPTOR。
