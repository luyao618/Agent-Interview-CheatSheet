...
2. **分数阈值（score threshold）**：只保留 rerank score ≥ τ 的条目。能按 query 难易自适应数量；但 reranker 的 raw / relevance score **通常不保证跨 query 校准**，全局阈值 τ 要用评测集标定，否则容易飘。
...
- **K 不是越大越好**：召回率随 K 升、但**信噪比下降**。无关片段会稀释注意力、诱发幻觉，即「context stuffing / lost-in-the-middle」；经验上常从 **K=3~5** 起步验证，实际拐点视语料密度、chunk 粒度、query 类型、模型上下文与评测指标而定。
...
不能直接当全局阈值用。reranker 的 raw / relevance score 通常不保证**跨 query 校准**——A 题的 0.6 和 B 题的 0.6 不一定可比，sigmoid/min-max 只是尺度映射、per-query min-max 也解决不了可比性。实践做法：① 全局阈值须用**带标注的检索评测集**（query→应命中片段）做校准/标定，对 (K, τ) 网格搜索，按 recall@K、nDCG、下游答案质量取**拐点**而非极值；② 线上优先用 **query 内相对分差** + `K_max` 兜底——头部有明显断崖就截，没断崖回退到 K_max。这样既自适应 query 难度，又不被分数漂移带偏。
