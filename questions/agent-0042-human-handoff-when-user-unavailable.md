...
需要人工接管
   ...
   ▼ 这一步是什么性质？
   ├─ 不可逆 / 高风险（转账、删库、外发）
   │     → 硬冻结：绝不代做，挂起等审批；超时只取消未执行动作，不自动回滚
...
落到四种可切换策略：**等待**（park：挂起到「等待人工」态，持久化现场后多渠道异步通知，而非占线程 busy-wait）、**降级继续**（safe subset：只做可逆低风险且无害的部分——预取数据、起草稿、dry-run 估影响，把待拍板动作留在门口）、**冻结**（freeze：高风险无法推进时停在 checkpoint、释放资源）、**兜底默认**（safe default：最终超时仍无人，走**预先约定**的默认——低危可放行；高危只取消尚未执行的动作，绝不「超时即自动回滚」，因为补偿/回滚本身也是有副作用的动作）。
...
t0 触发 handoff ─► 持久化现场 + 首次通知
   ...
   ├─ 硬超时 T2：执行预设安全默认
   │     ├─ 低危 → 放行
   │     └─ 高危 → 只取消尚未执行的动作；补偿/回滚须单独评估，
   │              仅在「预先授权 + 幂等 + 验证安全」时才自动执行，
   │              否则保持冻结、继续等人工
