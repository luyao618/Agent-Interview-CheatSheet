...
[返回项目首页](./README.md) · [在线题库](https://luyao.blog/ai-interview/)

欢迎补充新题、纠正答案、提供不同解法、添加来源或贡献英文翻译。题目正文维护在 `questions/`，索引维护在 `index.json`；`index.md` 和 `html/` 由脚本生成。
...
2. **新建文件**：复制下方[单题格式约定](#单题格式约定)中的模板，在 `questions/` 根目录下新建文件，文件名为 `<id>-<slug>.md`，`<slug>` 用英文 kebab-case，如 `agent-0007-tool-calling-design.md`。**不要创建分类子目录。**
...
6. **登记索引**：在 `index.json` 的 `questions` 数组中新增/更新对应 entry（字段见下方「索引文件」），确保 `file` 指向新建的 Markdown 文件。
7. **生成与检查**：按下方「生成与检查」更新目录页与 HTML，并完成校验。
8. **提交 PR**：将题目、索引与生成产物一起提交，遵循下方命名与提交规范。
...
- 提交信息建议遵循 [Conventional Commits](https://www.conventionalcommits.org/)：
  ...
  - `docs: fix answer in agent-0025-react-pattern-working-principle`（修正内容）
...
已有 Mermaid 图的 SVG 缓存随仓库提供。新增或修改 Mermaid 图时，先运行 `npm --prefix scripts/diagram-renderer ci` 安装渲染依赖，再生成 HTML。

网页样式和交互分别维护在 [单题模板](./templates/question/README.md) 与 [学习主页模板](./templates/workspace/README.md)，不要直接修改 `html/` 中的生成文件。界面调整后，按模板文档检查桌面端、手机端、作答保存与导航。

## 分类与文件约定

- 所有题目平铺在 `questions/`，每题一个 Markdown 文件，不创建分类子目录。
- `id`、文件名前缀与 `category` 保持一致，分类以 `index.json` 的 `categories` 为准。
- 技术题按 `llm` / `agent` / `rag` / `engineering` 归类；`product` 只放纯产品方法论题。适用岗位通过 `role` 单独标注。
- 英文翻译使用同 basename 的 `.en.md` 文件，并登记在索引的 `translations` 中。
- 题目正文是内容来源，索引只记录检索、分类和排序所需的元数据。
...
可复制下面的模板新增题目。示例 ID 和日期仅用于说明，请按当前索引分配未使用的 ID，并填写实际日期：
...
answers:                          # 多答案列表，至少一条
  - author: Claude-Opus-4.8       # 署名：AI 答案填模型名，人类答案填自取的名字
    type: ai                      # ai | human
    model: Claude-Opus-4.8        # 仅 AI 答案填，供索引维护与后续统计使用；人类答案省略
